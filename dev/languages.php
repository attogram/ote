<?php
// Open Translation Engine - Languages Page v0.3.3
...
foreach ($langs as $code => $lang) {
    $qr = $this->database->query(
        'SELECT count(distinct sw) AS count FROM word2word WHERE sl = :sl',
        array('sl'=>$lang['id'])
    );
    $num_words = isset($qr[0]['count']) ? $qr[0]['count'] : '0';
    $qr = $this->database->query(
        'SELECT count(sw) AS count FROM word2word WHERE sl = :sl',
        array('sl'=>$lang['id'])
    );
    ...
    print '<div class="row" style="border:1px solid #ccc;padding:8px;">'
        . '<div class="col-sm-3"><h2><kbd>' . $lang['name'] . '</kbd></h2></div>'
        . '<div class="col-sm-2"><h3><code>' . $code . '</code></h3></div>'
        . '<div class="col-sm-3">' . $num_dictionaries . ' dictionaries:<br />'
        . $dictionaries . '</div>'
        . '<div class="col-sm-2"><a href="' . $word_list_url . '">'
        . $num_words . '</a> words</div>'
        . '<div class="col-sm-2"><a href="' . $translations_url . '">'
        . $num_translations . '</a> translations</div>'
        . '</div>';
}
