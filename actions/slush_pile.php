  ...
  switch( $action ) {
    ...
    case 'd':  // Delete slush pile entry
      if( $ote->delete_from_slush_pile( $slush_id ) ) {
        print '<div class="alert alert-success"><a href="#" class="close" data-dismiss="alert" aria-label="close">&times;</a>'
        . '<strong>Deleted</strong>: Slush Pile ID #' . $this->web_display($slush_id) . '</div>';
      } else {
        print '<div class="alert alert-danger"><a href="#" class="close" data-dismiss="alert" aria-label="close">&times;</a>'
        . '<strong>Error</strong>: ' . $_SESSION['error'] . '</div>';
        unset($_SESSION['error']);
...
foreach( $slush as $s ) {
  ...
  $date = isset($s['date']) ? $this->web_display($s['date']) : '';
  $type = isset($s['type']) ? $this->web_display($s['type']) : '';
  $user_id = isset($s['user_id']) ? $this->web_display($s['user_id']) : '';
  $source_word = isset($s['source_word']) ? $this->web_display($s['source_word']) : '';
  $source_word_url = '../word///' . urlencode($s['source_word']);
  $source_language_code = isset($s['source_language_code']) ? $this->web_display($s['source_language_code']) : '';
  $target_word = isset($s['target_word']) ? $this->web_display($s['target_word']) : '';
